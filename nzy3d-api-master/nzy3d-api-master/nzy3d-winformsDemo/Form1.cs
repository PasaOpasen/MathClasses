...
namespace nzy3d_winformsDemo
{
    public partial class Form1 : Form
    {
        ...
        public Form1(string name="",double xmin=-150,double xmax=150,int xstep=50, double ymin = -150, double ymax = 150, int ystep = 50,Func<double,double,double> func= null)
        {
            ...
            this.xstep = xstep;
            this.ystep = ystep;
            if(func==null) func = (double x, double y) => x * x * Math.Cos(y) + y * y * Math.Cos(x);
            this.func = new Func<double, double, double>(func);
        ...
        private void InitRenderer()
        {
           ...
           // mainPanel.Controls.Clear();
            ...
            Range rangex = new Range(xmin, xmax),rangey=new Range(ymin,ymax);
            //int steps = 50;

            // Build a nice surface to display with cool alpha colors 
            // (alpha 0.8 for surface color and 0.5 for wireframe)
            Shape surface = Builder.buildOrthonomal(new OrthonormalGrid(rangex, xstep, rangey, ystep), new MyMapper(func));
